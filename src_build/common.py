"""Common build utilities (replaces hat.doit.common)."""
...
def init(python_paths: Iterable[os.PathLike] = [],
    ...
    """Initialize doit configuration."""
    add_python_paths(*python_paths)

    conf = {'backend': 'sqlite3',
            'default_tasks': default_tasks,
            'verbosity': verbosity,
            'num_process': _get_num_process()}

    if sys.version_info[:2] >= (3, 14):
        conf['par_type'] = 'thread'
...
def add_python_paths(*paths: os.PathLike):
    """Add paths to sys.path and PYTHONPATH."""
...
def get_task_json_schema_repo(src_paths: Iterable[Path],
    ...
    """Create a task that generates a JSON Schema Repository."""
    import hat.json
...
def get_task_copy(src_dst_paths: Iterable[tuple[Path, Path]],
    ...
    """Create a task that copies files/directories."""
...
def _get_num_process():
    """Get the number of parallel processes for doit."""
